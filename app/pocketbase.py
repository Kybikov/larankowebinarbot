...
class PocketBaseClient:
    ...
    async def ensure_schema(self) -> None:
        ...
        for collection_cfg in collection_definitions():
            name = collection_cfg["name"]
            if name not in existing:
                await self._request("POST", "/api/collections", json=collection_cfg)
                continue
            existing_collection = existing[name]
            existing_fields = {field["name"]: field for field in existing_collection.get("schema", [])}
            new_schema = list(existing_collection.get("schema", []))
            changed = False
            for field_cfg in collection_cfg["schema"]:
                fname = field_cfg["name"]
                if fname not in existing_fields:
                    new_schema.append(field_cfg)
                    changed = True
                elif field_cfg.get("type") == "select":
                    existing_field = existing_fields[fname]
                    existing_opts = set(existing_field.get("options", {}).get("values", []))
                    new_opts = set(field_cfg.get("options", {}).get("values", []))
                    if new_opts - existing_opts:
                        merged = list(existing_opts | new_opts)
                        for idx, f in enumerate(new_schema):
                            if f["name"] == fname:
                                new_schema[idx] = {**f, "options": {**f.get("options", {}), "values": merged}}
                                changed = True
                                break
            if changed:
                await self._request(
    ...
    definitions = [
        ...
        },
        {
            ...
            "schema": [
                ...
                {"name": "status", "type": "select", "required": True, "options": {"maxSelect": 1, "values": ["pending", "sending", "sent", "failed", "cancelled"]}},
